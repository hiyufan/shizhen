...
const WARM: &[alcedo::Source] = &[alcedo::Source::DouYin, alcedo::Source::BiliBili, alcedo::Source::RedBook];

pub fn build(cfg: Config) -> Result<Shared, String> {
    cfg.ensure_dirs().map_err(|e| format!("创建 {} 失败: {e}", cfg.data_dir.display()))?;
    let signer = Signer::load(cfg.secret.as_deref(), &cfg.data_dir).map_err(|e| format!("读写 secret.key 失败: {e}"))?;
    ...
    let threads = if cfg.ffmpeg_threads > 0 { cfg.ffmpeg_threads } else { (cpus / cfg.max_concurrent_jobs.max(1)).max(1) };
    let kit = Toolkit {
    ...
    let site = Site::new(Content::embedded()?, site_cfg).map_err(|e| format!("模板加载失败: {e}"))?;

    Ok(Arc::new(AppState {
        jobs: Jobs::new(cfg.max_concurrent_jobs, cfg.max_queued_jobs, cfg.job_timeout, Arc::clone(&stats), cfg.outputs_dir()),
...
pub fn spawn_background(state: &Shared) {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let _ = tokio::task::spawn_blocking(move || {
                Cleanup { cfg: &st.cfg, jobs: &st.jobs, store: &st.store, stats: &st.stats }.run();
            })
            .await;
    ...
    let every = if state.cfg.relay.is_some() { Duration::from_secs(60) } else { Duration::from_secs(240) };
    let s = Arc::clone(state);
    tokio::spawn(async move {
        loop {
...
pub async fn serve(state: Shared) -> Result<(), String> {
    ...
    let listener = tokio::net::TcpListener::bind(addr).await.map_err(|e| format!("监听 {addr} 失败: {e}"))?;
