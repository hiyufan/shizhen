...
static SPACES: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\s+").unwrap_or_else(|_| unreachable!()));
...
const FILENAME_SAFE: &AsciiSet = &NON_ALPHANUMERIC.remove(b'-').remove(b'.').remove(b'_').remove(b'~');

/// `Content-Disposition` 的值：中文文件名走 `filename*=UTF-8''...`。
pub fn attachment_header(name: &str) -> String {
    format!("attachment; filename*=UTF-8''{}", utf8_percent_encode(name, FILENAME_SAFE))
}
...
mod tests {
    ...
    fn filenames() {
        ...
        assert_eq!(safe_filename(&"长".repeat(100), ".jpg", "x").chars().count(), 64);
    }

    #[test]
    fn attachment_keeps_dots() {
        assert_eq!(attachment_header("a b.mp4"), "attachment; filename*=UTF-8''a%20b.mp4");
    ...
    fn paths_are_scrubbed() {
        ...
        assert_eq!(scrub_paths("open /srv/app/data/x.mp4 failed", dir), "open data/x.mp4 failed");
    }
}
